...
**Live book after D-054:** virtual −0.44 = broker −0.44, SHORT2 open, halt off. Do not `/reconcile` or `/rematch` while that remains true.
...
`Bot: RUNNING` prints only when both operator pause and safety halt are false. There is no `Safety halt:` line and no `Operator pause: ACTIVE` line unless those are latched.

This command never places an order. After D-054 it forces a monitor poll before printing broker lines. Unread broker net prints as unavailable, not as `0`.

### `/health`
Checks that the Railway worker and PostgreSQL are reachable, the SOL 200-day moving-average provider is available, and reports the current automatic-execution state. Also forces a monitor poll before broker lines.
...
**Forbidden while SHORT2 or any other real DXtrade SOL position is open.** That command empties virtual lots. It is not how you clear an unread-book scare or a matched-book halt.
...
This command is not an alternate `/resume`. It is refused when there is no safety halt, and refused when any other halt is active — including a different halt whose wording happens to contain “reconcil”. With halt off and books already matched, `/rematch` will refuse. That is correct.
...
BMTB1 on `main` (`9438cf73`) can speak as the project, read the code-body map, read five sticky operator latches, and call read-only GitHub tools (D-052) when `GITHUB_TOKEN` is on the companion worker. He cannot see live fills unless the owner latched `/status` or D-051 lands. He cannot place orders, clear a halt, merge, or deploy.
...
Owner Telegram message
  ...
  -> OpenAI Responses API (+ optional D-052 GitHub tools)
...
The OpenAI API key and `GITHUB_TOKEN` exist only in the companion worker. The companion worker does not hold the Telegram bot token or DXtrade credentials.

D-051 live snapshot publish/read is approved and not on `main`. Until that lands, live body facts come from latched `/status` (and the other four sticky slots), not from a Postgres snapshot row.

Phase 2d proposal-bound GitHub writes are not implemented and are not next.
...
- `🚨 SOL SAFETY HALT — RECONCILIATION MISMATCH` — virtual net SOL and a **finite** DXtrade net SOL disagree outside the reconciliation grace window;
...
Unread broker data (D-054) does **not** raise the reconciliation-mismatch halt.
...
The worker keeps each ring fill as a durable virtual lot while the DXtrade account remains netted. If the signed total of virtual SOL inventory no longer reconciles to a **finite** broker net SOL position, new strategy actions fail closed and require owner review. A missing snapshot is unread (`ACCOUNT_DATA_UNAVAILABLE`), not net zero.
...
- Broker net unavailable / unread — D-054. Not a mismatch. Do not `/reconcile`. Check trading-worker logs if it stays unread after `/status`.
- Finite reconciliation mismatch — confirm the actual DXtrade `SOL/USD` net. If halt is latched and nets now agree, `/rematch` then `/confirmrematch CODE`. If DXtrade is actually flat and only virtual lots remain, `/reconcile` then `/confirmreconcile CODE`.
- SHORT2 (or any open broker SOL) still open — never `/reconcile`.
- `/rematch` refused with halt off — expected. Rematch is not a general resume.
