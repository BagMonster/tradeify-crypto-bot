...
The canary response must now distinguish the broker state without submitting another order:
...
- if the account is flat, keep the failed/pending canary state visible so the execution-path cause can be corrected before a new uniquely versioned canary is authorized by the already-approved lifecycle objective.
