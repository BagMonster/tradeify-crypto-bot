#!/usr/bin/env node
/**
 * scripts/zone-tp-adx-sweep.mjs
 *
 * Same high-trade zone + rejection engine as zone-tp-sweep.mjs
 * with one added gate: previous day's ADX ≤ 25.
 *
...
const STOP_PCT = 0.05;
const ADX_MAX = 25;               // <-- the only new filter
const TP_PCTS = [
...
/**
 * Build a map: dayKey → previous day's ADX (no lookahead)
 */
function buildPrevDayADX(dailyBars, period = 14) {
  ...
  for (let i = 1; i < dailyBars.length; i++) {
    const prevAdx = adxValues[i - 1]; // previous completed day
    if (prevAdx != null) {
...
function runConfig({ side, zoneFrac, tpPct }, bars5m, boxByDay, prevAdxByDay) {
  const trades = [];
  const openTrades = [];
  let currentDay = null;
  ...
  for (let i = 2; i < bars5m.length; i += 1) {
    ...
    // ----- ADX gate (previous day only) -----
    const prevAdx = prevAdxByDay.get(day);
    if (prevAdx == null || prevAdx > ADX_MAX) {
      skippedAdx += 1;
      continue;
    }
...
async function main() {
  ...
  console.log(`Stop=${STOP_PCT * 100}% | ADX ≤ ${ADX_MAX} (previous day)`);
  ...
  for (const side of SIDES) {
    for (const zoneFrac of ZONE_FRACS) {
      for (const tpPct of TP_PCTS) {
        const r = runConfig({ side, zoneFrac, tpPct }, bars5m, boxByDay, prevAdxByDay);
        results.push(r);
        console.log(
          ...
          `skippedAdx=${r.skippedAdx} skippedBudget=${r.skippedBudget}`
        );
      }
    }
  }
  ...
  console.log("\n=== Best by avgR (min 50 trades) ===");
  for (const r of [...results].filter((x) => x.trades >= 50).sort((a, b) => b.avgR - a.avgR).slice(0, 20)) {
    console.log(
      `${r.side}/zone${r.zonePct}%/tp${r.tpPct}%: ` +
  ...
  const outPath = path.join(OUT_DIR, "zone-tp-adx-sweep.json");
  await writeFile(
    outPath,
    `${JSON.stringify({ generatedAt: new Date().toISOString(), adxMax: ADX_MAX, results }, null, 2)}\n`,
    "utf8"
  );
  console.log(`\nWrote ${outPath}`);
