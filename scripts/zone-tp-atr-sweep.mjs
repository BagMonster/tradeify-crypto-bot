#!/usr/bin/env node
/**
 * scripts/zone-tp-atr-sweep.mjs
 *
 * High-trade zone + rejection engine
 * + rolling 120-day ATR% percentile gate (no ADX)
 *
 * Tests three bands:
 *   40–60, 35–70, 30–75
 *
...
/**
 * Compute rolling 120-day ATR% percentile using only prior days.
 * Returns Map: dayKey → percentile (0-100) of the previous day.
 */
function buildPrevDayAtrPercentile(dailyBars, window = 120) {
  ...
  for (let i = 1; i < n; i++) {
    // Use days [i-window ... i-1]  (prior days only)
    const start = Math.max(0, i - window);
    const slice = atrPct.slice(start, i); // excludes current day i

    if (slice.length < 20) continue; // need minimum history

    const sorted = [...slice].sort((a, b) => a - b);
    const value = atrPct[i - 1]; // previous day's ATR%
    // percentile rank
    let rank = 0;
...
function runConfig({ side, zoneFrac, tpPct, atrLo, atrHi }, bars5m, boxByDay, prevAtrPctByDay) {
  const trades = [];
  const openTrades = [];
  let currentDay = null;
  ...
  for (let i = 2; i < bars5m.length; i += 1) {
    ...
    // ----- ATR% percentile gate (previous day only) -----
    const prevPct = prevAtrPctByDay.get(day);
    if (prevPct == null || prevPct < atrLo || prevPct > atrHi) {
      skippedAtr += 1;
      continue;
    }
...
async function main() {
  ...
  console.log(`Stop=${STOP_PCT * 100}% | ATR% window=${ATR_WINDOW} days (prior only)`);
      ...
      for (const zoneFrac of ZONE_FRACS) {
        for (const tpPct of TP_PCTS) {
          const r = runConfig(
            ...
            bars5m, boxByDay, prevAtrPctByDay
          );
          results.push(r);
          console.log(
            ...
            `stopRate=${(r.stopRate * 100).toFixed(1)}% skippedAtr=${r.skippedAtr} skippedBudget=${r.skippedBudget}`
          );
        }
      }
    }
  ...
  console.log("\n=== Best by avgR (min 40 trades) ===");
  for (const r of [...results].filter((x) => x.trades >= 40).sort((a, b) => b.avgR - a.avgR).slice(0, 25)) {
    console.log(
      `${r.side}/zone${r.zonePct}%/tp${r.tpPct}% [${r.atrBand}]: ` +
  ...
  const outPath = path.join(OUT_DIR, "zone-tp-atr-sweep.json");
  await writeFile(
    outPath,
    `${JSON.stringify({ generatedAt: new Date().toISOString(), atrWindow: ATR_WINDOW, bands: ATR_BANDS, results }, null, 2)}\n`,
    "utf8"
  );
  console.log(`\nWrote ${outPath}`);
