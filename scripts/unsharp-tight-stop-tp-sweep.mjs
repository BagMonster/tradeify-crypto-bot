#!/usr/bin/env node
/**
 * scripts/unsharp-tight-stop-tp-sweep.mjs
 *
 * Unsharp 3-candle entry
 * + tighter stop range (1.0% → 3.5%)
 * + lower TP range (starting just above cost)
 *
 * Goal: test whether higher notional + small achievable TPs
 * can produce better expectancy than the wide-stop versions.
 *
...
const STOP_PCTS = [0.01, 0.015, 0.02, 0.025, 0.03, 0.035];
...
async function main() {
  ...
  console.log("\n=== Best by totalR ===");
  for (const r of [...results].sort((a, b) => b.totalR - a.totalR).slice(0, 30)) {
    console.log(
      `${r.side}/stop${r.stopPct}%/tp${r.tpPct}%: ` +
  ...
  for (const r of [...results].filter((x) => x.trades >= 40).sort((a, b) => b.avgR - a.avgR).slice(0, 30)) {
    console.log(
      `${r.side}/stop${r.stopPct}%/tp${r.tpPct}%: ` +
  ...
  for (const side of SIDES) {
    ...
    for (const stopPct of STOP_PCTS) {
      ...
      const avgTpRate = subset.reduce((s, r) => s + r.tpRate, 0) / subset.length;
      ...
      console.log(
        ...
        `avgStopRate=${(avgStopRate * 100).toFixed(1)}%  avgTpRate=${(avgTpRate * 100).toFixed(1)}%  ` +
        `avgTimeRate=${(avgTimeRate * 100).toFixed(1)}%  avgR=${avgAvgR.toFixed(3)}  avgNotional=$${avgNotional.toFixed(0)}`
      );
    }
  }

  await mkdir(OUT_DIR, { recursive: true });
  const outPath = path.join(OUT_DIR, "unsharp-tight-stop-tp-sweep.json");
  await writeFile(
    outPath,
