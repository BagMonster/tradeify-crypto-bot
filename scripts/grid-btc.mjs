...
function simulate(bars, cfg, keepTrades = false) {
  ...
  for (let i = 1; i < bars.length; i += 1) {
    ...
    const dayDelta = (realised - dayStartRealised) + unrealised;
    if (dayDelta < worstDay) worstDay = dayDelta;
    if (equity <= cfg.maxLossFloor && !mllTouched) {
  ...
  return {
    ...
    breaches, firstBreach, mllTouched, worstDay, peakNotional, skippedNotional,
